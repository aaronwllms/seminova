...
- **The "Resolved decisions" block** at the bottom of the archive is a separate small migration: those are decisions-with-rationale → ADR candidates. Ones clearing the three-part bar become ADRs; the rest stay as a plain decisions log. Not part of the freeze.
...
This is recorded as the destination if the Phase-7 fork ever goes that way. **It is not what the first chunk builds.** The first chunk's pipeline is: `phase-planning` (grill + epics/stories with vertical-slice + success-condition discipline) → `plan-next-epic` (plan one epic, build to the success condition) → `plan-review` → ship.
...
- **Doc-governance homes are split by trigger mechanism, NOT collapsed into one file (decided 2026-06-27).** This was re-opened once; the full reasoning is recorded here so it does not have to be re-litigated.
  ...
  **The clean resolution — assign each piece to a home by its trigger/audience:**
  ...
  - **The duplicated roles table** (`DOC_RULES.md` ↔ `AGENTS.md`) → collapse to a single home.
...
- **Skill edits not yet applied** (recorded as intent, execute in Phase 4): `phase-planning` slice + success-condition + size-cap; `plan-next-epic` self-contained-plan tightening.
- **Doc-governance rework not yet applied** (decided §6, execute in Phase 4): thin-pointer `documentation.mdc` rewrite; `DOC_RULES.md` rationalization to procedure-only; roles-table dedup (`DOC_RULES.md` ↔ `AGENTS.md`).
- **Net-new artifacts:** LEXICON.md ✅ and `docs/prds/` (+ README) ✅ done. Remaining: the **first real PRD** — awaits a genuine phase promotion (not created on a manufactured phase).
- **Moves not yet done:** `CONTEXT_ARCHIVE.md` → `docs/archive/`; `DOC_RULES.md` → `docs/`.
...
- **Claude side: the repo branch doesn't cover Claude-side state** (project instructions; skills at `/mnt/skills/user/`). Only the repo has branches.
  ...
  - **Additive-only for skills.** Leave existing skills untouched; add new ones alongside. No rename-to-legacy, no archival — nothing is rewired out from under `main` yet. (Rename-to-legacy stays available as a fallback if a future change must repoint an existing skill destructively.)
...
- Doc maintenance rules (to move → `docs/`, then rewrite): `/Users/aaronwilliams/projects/seminova/DOC_RULES.md`
