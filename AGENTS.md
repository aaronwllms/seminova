...
- **Auth boundary** — public routes are `/`, `/auth/**`, `/terms`, and `/privacy`; all others require a session; enforced in `proxy.ts` → `src/supabase/proxy.ts`. **Enforced:** `check:auth-boundary` (discovered-route proxy tests).
...
| Path | Purpose |
| ---- | ------- |
| `src/app/(marketing)/` | Public marketing routes (`/`, `/terms`, `/privacy`) |
