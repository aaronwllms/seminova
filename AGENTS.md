...
| Admin user list | `public.admin_list_users` | `SECURITY DEFINER` RPC — paginated, sortable admin listing over `auth.users` (email, verification, created, last sign-in, role, `banned_until`); optional `p_filter_unverified` and `p_filter_banned` tile filters (both false = unfiltered; may be active together; unified `is_verified` / `is_currently_banned` predicates shared by filters and sort); in-function admin gate via `auth.jwt()` `app_metadata.role`; static sort allowlist; `EXECUTE` granted to `authenticated` only. Called from [`list-admin-users.ts`](src/app/admin/users/_lib/list-admin-users.ts). |
| Admin user stats | `public.admin_user_stats` | `SECURITY DEFINER` RPC — global total, unverified, and currently-banned counts over `auth.users` for users page stat tiles; same admin gate and verification/banned predicates as `admin_list_users`; counts unaffected by list filters or search; `EXECUTE` granted to `authenticated` only. Called from [`list-admin-user-stats.ts`](src/app/admin/users/_lib/list-admin-user-stats.ts). |
