...
Six custom migrations — filenames and per-table detail in [Data model](#data-model-summary) below. `avatars` storage bucket is public-read with owner-scoped write RLS. Types generated to [`src/types/database.types.ts`](src/types/database.types.ts) via `pnpm db:types`; domain aliases in [`src/types/profile.ts`](src/types/profile.ts) and [`src/types/app-settings.ts`](src/types/app-settings.ts).
