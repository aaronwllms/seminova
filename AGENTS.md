...
- **Auth boundary** — public routes are `/`, `/auth/**`, `/terms`, `/privacy`, `/reference`, `/features`, `/workflow`, and `/api/client-logs` (client log relay — see [ADR-0007](docs/adr/ADR-0007-client-log-relay-unauthenticated.md)); all others require a session; enforced in `src/proxy.ts` → `src/supabase/proxy.ts`. **Enforced:** `check:auth-boundary` (discovered-route proxy tests).
...
Supabase email/password flows under `/auth/**` (login, sign-up, forgot/update password, confirm, error) with shared auth layout. `src/proxy.ts` → `src/supabase/proxy.ts` is the **server-side session gate**: it refreshes sessions on matched HTTP requests; the browser client ([`client.ts`](src/supabase/client.ts)) uses default foreground auto-refresh for Client Components. On protected routes without a session, an ordinary logged-out request redirects to `/auth/login` (with safe `next` preservation via [`buildLoginRedirectUrl`](src/utils/build-login-redirect-url.ts)), while a request carrying a stray auth `code` param logs a diagnostic and redirects to `/auth/error?source=stray_code` instead; non-admins on `/admin/**` redirect to `/home`. When public Supabase env is unset, production returns **503** for all routes; in development, public routes (`/`, `/auth/**`, `/terms`, `/privacy`, `/reference`, `/features`, `/workflow`, `/api/client-logs`) pass through and **protected routes return 503** — RSC reads no longer gate auth. Post-login redirect: admins → `/admin`, non-admins → `/home` (`getPostAuthRedirectPath` in [`admin.ts`](src/utils/admin.ts); `APP_HOME` in [`app-paths.ts`](src/constants/app-paths.ts)); the login form and [`/auth/confirm`](src/app/auth/confirm/route.ts) honor a safe `next` query param via [`isSafeRedirect`](src/utils/is-safe-redirect.ts). **Read vs mutation split:** layouts, server components, and gates (`AdminAuthGate`, `getCurrentUserProfile`) read via `getDisplayAuthClaims` / `hasServerAuthSession` from [`require-auth.ts`](src/supabase/require-auth.ts) — [`AdminAuthGate`](src/app/admin/_components/admin-auth-gate.tsx) probes `hasServerAuthSession` first and renders [`AdminShellSkeleton`](src/app/admin/_components/admin-shell-skeleton.tsx) when the probe fails (cold cache / logged-out) before calling `getDisplayAuthClaims` — these validate the cookie-read access token via `getClaims(accessToken, { allowExpired: true })` (signature-verified, exp-tolerated, never refresh); missing or invalid tokens on protected routes throw (proxy should have gated). Server actions use `getUser()` at the trust boundary so the Auth server validates the access token (see [`updateProfileAction`](src/app/(app)/_lib/profile/actions.ts) and [`forms.mdc`](.cursor/rules/forms.mdc) step 1). Do not call bare `getClaims()` or `getSession()` on RSC read paths — always pass an explicit token. Refresh uses the two-authority model — proxy on matched server requests plus browser foreground auto-refresh ([ADR-0005](docs/adr/ADR-0005-proxy-session-gate-two-authority-refresh.md)). At **debug** threshold (`min_log_level`), proxy emits refresh-vs-reuse on protected happy paths; `getDisplayAuthClaims` / `hasServerAuthSession` emit display-read and expired-token probe lines (JWT `exp` decoded in [`read-auth-cookie.ts`](src/supabase/read-auth-cookie.ts) — never log tokens). Idle-tab avatar uploads use `probeSessionAction` + one retry after `router.refresh()` — not a second refresh authority. [`extractAuthFormError`](src/utils/extract-auth-form-error.ts) is fallback-first — enumerated `AUTH_ERROR_OVERRIDES` for known Supabase codes, generic operational copy for unmapped codes, never raw Supabase messages. Four auth forms carry `autocomplete` tokens per [`forms.mdc`](.cursor/rules/forms.mdc); update-password includes a hidden paired username field from the recovery session.

### Admin console

`/admin` route segment — dashboard landing plus `/admin/users`, `/admin/logs`, and `/admin/settings` — gated by `src/proxy.ts` + [`AdminAuthGate`](src/app/admin/_components/admin-auth-gate.tsx); sidebar layout (`sidebar-07` baseline) with sidebar-header [`SeminovaLogo`](src/components/seminova-logo.tsx) linking to marketing home (`/`), not the admin dashboard; dynamic breadcrumb; path constants in [`src/constants/admin-paths.ts`](src/constants/admin-paths.ts). Admin identity loads from [`getCurrentUserProfile`](src/app/(app)/_lib/get-current-user-profile.ts) in `AdminAuthGate`; [`AdminShell`](src/app/admin/_components/admin-shell.tsx) wraps [`ProfileDialogProvider`](src/app/(app)/_components/profile/profile-dialog-provider.tsx) so the sidebar can open profile settings modally. Admin CLI (`pnpm promote-admin` / `demote-admin` / `delete-user` / `list-admins`, secret-key scripts in `scripts/admin/`) and in-app promote/demote and ban/unban (row actions on `/admin/users` with confirmation dialogs + toasts; ban uses a fixed duration set including permanent, self-ban blocked, unban idempotent) share mutation logic in [`src/utils/admin-user-mutations.ts`](src/utils/admin-user-mutations.ts). Users table lists real Supabase Auth users via a gated Server Action (caller gate in [`assert-admin-caller.ts`](src/app/admin/users/_lib/assert-admin-caller.ts)) + `admin_list_users` RPC (migration-shipped `SECURITY DEFINER` function reading `auth.users` with in-function admin gate and static sort allowlist); stat-tile filters above the table (Total, Unverified, Banned, New (30d)) built on shared [`StatTile`](src/components/stat-tile.tsx) and [`useToggleFilterSet`](src/hooks/use-toggle-filter-set.ts) — global counts via [`getUserStatsAction`](src/app/admin/users/actions.ts) / `admin_user_stats` RPC unaffected by search or other tile state; Total clears all filters (tiles + email search) and shows selected only when fully unfiltered; tile filters toggle independently and compose with debounced email search (≥3 chars) and paging; toolbar [`UsersToolbar`](src/app/admin/users/_components/users-toolbar.tsx) (icon-in-field search, inline [`UsersActiveFilters`](src/app/admin/users/_components/users-active-filters.tsx) chips when any filter/search active, manual refresh); filtered zero results via [`UsersFilteredEmptyState`](src/app/admin/users/_components/users-filtered-empty-state.tsx) (Reset filters + Refresh); manual refresh via [`useAdminUsersRefresh`](src/app/admin/users/_lib/use-admin-users-refresh.ts) refetches list + stats alongside existing focus refetch; stat tiles skeleton while stats load (no zero flash); role column shows Admin or User badges; server-driven sort on all six columns, configurable page size (10 / 15 / 25 / 50, default 15) via shared [`DataTablePaginationControls`](src/components/data-table-pagination-controls.tsx), canonical data-table pattern (see [`.cursor/rules/data-tables.mdc`](.cursor/rules/data-tables.mdc)) with per-column width/alignment via column meta `cellClassName` and skeleton shape via `skeletonClassName` through [`DataTableShell`](src/components/data-table-shell.tsx) + [`DataTableSkeletonBody`](src/components/data-table-skeleton-body.tsx); stale-row refetch dim via shared [`TableFetchDimWrapper`](src/components/table-fetch-dim-wrapper.tsx) / [`useTableFetchDim`](src/hooks/use-table-fetch-dim.ts) (minimum hold matching `duration-swept`; `aria-busy` tracks fetch only); list and stat-tile counts refetch on window focus via [`useAdminUsersList`](src/app/admin/users/_lib/use-admin-users-list.ts) and [`useAdminUserStats`](src/app/admin/users/_lib/use-admin-user-stats.ts) (`refetchOnWindowFocus: 'always'` — lighter freshness tier per [ADR-0008](docs/adr/ADR-0008-realtime-scoped-to-logs-tiered-freshness.md); manual refresh as catch-up safety valve; no live feed or connection indicator). [`/admin/logs`](src/app/admin/logs/page.tsx) lists persisted `app_logs` rows with a live feed via Supabase Realtime — [`useAdminLogsRealtime`](src/app/admin/logs/_lib/use-admin-logs-realtime.ts) subscribes to INSERT-only events, debounced invalidate-and-refetch through TanStack Query (respects active filters; no client-side row merge per [ADR-0008](docs/adr/ADR-0008-realtime-scoped-to-logs-tiered-freshness.md)); toolbar [`LogsToolbar`](src/app/admin/logs/_components/logs-toolbar.tsx) (free-text search, inline [`LogsActiveFilters`](src/app/admin/logs/_components/logs-active-filters.tsx) chips when any filter/search active, [`LogsLiveToggle`](src/app/admin/logs/_components/logs-live-toggle.tsx) play/pause — default off; preference persisted in localStorage per [ADR-0009](docs/adr/ADR-0009-client-ui-preferences-localstorage-default.md) — and manual refresh with spinner); live toggle subscribes or unsubscribes from the Realtime feed; manual refresh forces catch-up refetch; stale-row refetch dim via the same shared [`TableFetchDimWrapper`](src/components/table-fetch-dim-wrapper.tsx); filtered empty view via [`LogsFilteredEmptyState`](src/app/admin/logs/_components/logs-filtered-empty-state.tsx) offers Reset filters and Refresh — plus stat-tile filters (Total, level tiles, Unread), a searchable tag combobox, free-text search across message/context/tag, and global read state — admin-gated [`listLogsAction`](src/app/admin/logs/actions.ts), [`getLogStatsAction`](src/app/admin/logs/actions.ts), [`listLogTagsAction`](src/app/admin/logs/actions.ts), [`markLogReadAction`](src/app/admin/logs/actions.ts), [`markLogUnreadAction`](src/app/admin/logs/actions.ts), and [`markAllLogsReadAction`](src/app/admin/logs/actions.ts); shared [`StatTile`](src/components/stat-tile.tsx) and [`useToggleFilterSet`](src/hooks/use-toggle-filter-set.ts) primitives; cursor paging on `(created_at, id)`; timestamp-only sort direction toggle; timestamps render in the browser local timezone via [`formatLogTimestampDisplay`](src/app/admin/logs/_lib/format-log-timestamp-display.ts); unread rows use the `primary` semantic token for row tint and leading dot; row click opens a detail dialog (marking unread rows read) and dot click marks read without opening; copy on an unread row marks read without opening; mark-all-read scoped to the active filter view; detail dialog includes Mark unread for read rows; full message and JSON context in the dialog; row-level and modal copy buttons emit pretty-printed structured JSON via [`buildLogRowCopyText`](src/app/admin/logs/_lib/build-log-row-copy-text.ts) (delegates to [`buildStructuredCopyText`](src/utils/build-structured-copy-text.ts); Copy → Copied affordance). [`/admin/settings`](src/app/admin/settings/page.tsx) renders registry-driven runtime configuration grouped by feature area — logging rows use type-matched controls (log level select, positive-integer input) with per-row Save via [`saveAppSettingAction`](src/app/admin/settings/_lib/actions.ts) and toast confirmation; banner rows live in [`BannerSettingsSection`](src/app/admin/settings/_components/banner-settings-section.tsx) with accordion expand/collapse, status badges, schedule fields, live preview, and per-row Save (same action); successful saves log cache invalidation at debug before `revalidateTag`; initial values load server-side via [`getResolvedAppSettings`](src/utils/app-settings.ts). Sidebar nav includes Users, Logs, and Settings. [`AdminNavUser`](src/app/admin/_components/admin-nav-user.tsx) sidebar button shows display name (email fallback), correct initials, and avatar image via shared [`UserAvatar`](src/components/user-avatar.tsx); dropdown offers Profile, Open app (`APP_HOME`), Light/Dark/System theme picker (shared [`ThemeDropdownMenuItems`](src/components/theme-dropdown-menu-items.tsx)), and sign-out. [`AppNavUser`](src/app/(app)/_components/app-nav-user.tsx) header trigger is avatar-only (same `UserAvatar`); dropdown opens profile settings in a dialog, adds an admin-gated Admin console link for admins when applicable, and includes the same shared theme picker — `isAdmin` derived server-side via `isAdminFromAppMetadata`.

### App home & profile

[`/home`](src/app/(app)/home/page.tsx) is the authenticated landing surface under the `(app)` shell — a minimal placeholder that signposts the public [`/reference`](src/app/(marketing)/reference/page.tsx) pattern page and [`/workflow`](src/app/(marketing)/workflow/page.tsx) explainer, and is meant to be replaced when spinning off. Profile settings are **modal-only** — there is no `/profile` route ([ADR-0004](docs/adr/ADR-0004-profile-modal-app-home-divergence.md)). [`AppShell`](src/app/(app)/_components/app-shell.tsx) wraps children in [`ProfileDialogProvider`](src/app/(app)/_components/profile/profile-dialog-provider.tsx) and uses shared [`SiteHeader`](src/components/site-header.tsx) / [`SiteFooter`](src/components/site-footer.tsx) with default [`SeminovaLogo`](src/components/seminova-logo.tsx) href `/` (marketing home); the account menu opens [`ProfileSettingsDialog`](src/app/(app)/_components/profile/profile-settings-dialog.tsx) via [`useProfileDialog`](src/app/(app)/_components/profile/profile-dialog-provider.tsx). Settings use `react-hook-form` + zod (see [`forms.mdc`](.cursor/rules/forms.mdc)). `public.profiles` is 1:1 with `auth.users`, owner-scoped RLS, auto-created via a signup trigger; no `role` column (admin gate stays on `app_metadata.role`). Display name and bio are blur-save (bio max 160 characters) with per-field in-flight guards and inline [`FieldSaveIndicator`](src/components/field-save-indicator.tsx) via [`useBlurSaveField`](src/hooks/use-blur-save-field.ts) and [`BlurSaveTextField`](src/components/blur-save-text-field.tsx); email is a read-only display field (change not supported). Avatar upload is upload-on-complete (validate → resize to WebP 256px cap → fixed `{userId}/avatar.webp` path → versioned public URL via `withAvatarCacheBust`; client pipeline in [`avatar-storage.ts`](src/utils/avatar-storage.ts)) with Change and Remove controls — removal clears `avatar_url` and deletes the storage object (storage delete failure does not block the profile update). At debug threshold, upload success logs via [`clientLog`](src/utils/client-logger.ts) (relay tag `client-avatar-storage`); server-side delete success and non-blocking delete failure log via `appLog` in [`updateProfileAction`](src/app/(app)/_lib/profile/actions.ts). Appearance (theme toggle via [`ProfileThemeSegment`](src/app/(app)/_components/profile/profile-theme-segment.tsx) — outline `ToggleGroup` Light/Dark/System) renders above password change, which is collapsed behind an accordion via [`ProfilePasswordSection`](src/app/(app)/_components/profile/profile-password-section.tsx) requiring `current_password`. Partial-only [`updateProfileAction`](src/app/(app)/_lib/profile/actions.ts) persists `avatar_url` only for owned storage paths via [`isOwnedAvatarStorageUrl`](src/utils/avatar-cache-bust.ts) — external or other-user URLs are omitted. Shared [`useSignOut`](src/hooks/use-sign-out.ts) hook used by `AppNavUser`, `AdminNavUser`, and [`LogoutButton`](src/components/logout-button.tsx).

### Landing page

`(marketing)` route group at `/`: hero, six-card **highlight reel** (`id="features"`) sourced from [`features-content.ts`](src/config/features-content.ts) (`homeHighlight` capabilities — display-only, no per-card links; eyebrow "Feature highlights," heading "The pieces you'd otherwise build first") with a **"See all features"** text link to `/features` below the grid (section chrome and CTA from [`landing-content.ts`](src/config/landing-content.ts) `featureHighlights`), a proof CTA band ([`LandingProofCta`](src/app/(marketing)/_components/landing-proof-cta.tsx) — muted background, two-link row to `/reference` and `/workflow` between features and tech stack), and a tech-stack marquee (six logos, icon+name cells, edge fades, pause on hover; SVG assets in `public/tech/`; marquee primitive in [`src/components/kibo-ui/marquee/`](src/components/kibo-ui/marquee/)). Public pattern reference at [`/reference`](src/app/(marketing)/reference/page.tsx) uses in-page section nav ([`ReferenceSectionNav`](src/app/(marketing)/reference/_components/reference-section-nav.tsx)) over tabbed design-token panels (Colors, Typography, Icons via [`ReferenceDesignSystemTabs`](src/app/(marketing)/reference/_components/reference-design-system-tabs.tsx) in [`ReferenceDesignSystemSection`](src/app/(marketing)/reference/_components/reference-design-system-section.tsx)), a Forms section with a live profile-settings preview (blur-save, upload-on-complete, coupled explicit submit via [`ReferenceProfileSettingsPreview`](src/app/(marketing)/reference/_components/reference-profile-settings-preview.tsx)) plus compact per-row and per-section admin-style save fixtures ([`ReferencePerRowFormDemo`](src/app/(marketing)/reference/_components/reference-per-row-form-demo.tsx), [`ReferencePerSectionFormDemo`](src/app/(marketing)/reference/_components/reference-per-section-form-demo.tsx) — mock persist only), error surfaces, toast variants (via shared [`toast-icon-config`](src/utils/toast-icon-config.tsx)), and the canonical data table over fixture data (production list-view shell parity — stat-tile filters, search, inline filter chips, refresh; client-side sort and pagination via shared [`DataTablePaginationControls`](src/components/data-table-pagination-controls.tsx)) — prose sections stay narrow while the table demo renders wide. Public workflow explainer at [`/workflow`](src/app/(marketing)/workflow/page.tsx) uses matching section nav ([`WorkflowSectionNav`](src/app/(marketing)/workflow/_components/workflow-section-nav.tsx)), two-environment ownership cards with official tool logos (`public/images/logos/`), an interactive plan-review-build loop ([`WorkflowDiagram`](src/app/(marketing)/workflow/_components/workflow-diagram.tsx) — hover/focus reveals skill and environment per step), a documents table with real paths and writer/reader roles, CI-enforced hard constraints, and a go-deeper CTA to the workflow guide on GitHub — prose stays narrow while the diagram and cards render wide. Public features inventory at [`/features`](src/app/(marketing)/features/page.tsx) renders categorized capability cards from [`src/config/features-content.ts`](src/config/features-content.ts), each linking to its `/reference` section anchor where a live demo exists — intro prose narrow, card grids wide. Public legal placeholders at [`/terms`](src/app/(marketing)/terms/page.tsx) and [`/privacy`](src/app/(marketing)/privacy/page.tsx) share [`LegalPlaceholder`](src/app/(marketing)/_components/legal-placeholder.tsx) — spinoff replaces with real policy copy; each page shows an App Privacy Policy Generator callout and a not-legal-advice disclaimer (terms notes the generator produces both policy types). Features nav href is `/features` in [`site.ts`](src/config/site.ts); home section keeps `id="features"` for in-page scroll only (e.g. direct `/#features` deep links). Shared chrome — [`SiteHeader`](src/components/site-header.tsx), [`SiteFooter`](src/components/site-footer.tsx), [`SiteContainer`](src/components/site-container.tsx) (`max-w-7xl`), [`SiteNavLinks`](src/components/site-nav-links.tsx) (pathname-based current-page styling with `aria-current` on the matched link; in-page hash links stay inactive on `/`), `SiteCopyright` — is reused by both the marketing surface and the `(app)` shell; [`SeminovaLogo`](src/components/seminova-logo.tsx) (icon + wordmark, single link) in header/footer links to marketing home (`/`) on every surface; authenticated return to app home is via account-menu Open app on marketing/admin surfaces, not a separate footer link; header and footer use a matched three-column `minmax` grid so nav links stay viewport-centered regardless of unequal flanking content. Session-aware marketing header via [`LandingHeader`](src/app/(marketing)/_components/landing-header.tsx): desktop `rightSlot` and mobile chrome both use Suspense with [`AppNavUserSkeleton`](src/app/(app)/_components/app-nav-user-skeleton.tsx) fallbacks (desktop prioritizes signed-in continuity over anonymous first-paint layout shift). [`LandingAuthSlot`](src/app/(marketing)/_components/landing-auth-slot.tsx) probes session with `hasServerAuthSession` (read mechanics in [Auth & session](#auth--session) above) — anonymous visitors get Sign in / Sign up; signed-in visitors get shared [`AppHeaderAccountNav`](src/app/(app)/_components/app-header-account-nav.tsx) with `showOpenApp` (avatar account menu: Profile, Open app, Admin console when admin, Light/Dark/System theme, Sign out) wrapped in [`ProfileDialogProvider`](src/app/(app)/_components/profile/profile-dialog-provider.tsx). Mobile signed-in chrome is [`LandingMobileHeaderChrome`](src/app/(marketing)/_components/landing-mobile-header-chrome.tsx) — hamburger for site nav plus header avatar; anonymous mobile keeps auth CTAs inside the sheet only. Marketing header avatar repaints after profile save via `persistField` → `router.refresh()` (`updateProfileAction` revalidates `(app)` layout only). All user-visible identity (name, description, logo, nav/social/legal links) is driven from [`src/config/site.ts`](src/config/site.ts); hero/proof-cta/tech-stack copy and highlight-reel section chrome live in [`src/config/landing-content.ts`](src/config/landing-content.ts); highlight **items** and the full features inventory live in [`src/config/features-content.ts`](src/config/features-content.ts) — re-skin from these three files. Marketing chrome (banner + header) sticks on scroll via [`MarketingStickyChrome`](src/app/(marketing)/_components/marketing-sticky-chrome.tsx) in [`(marketing)/layout.tsx`](src/app/(marketing)/layout.tsx) — when [`banner_public`](src/config/app-settings-registry.ts) is live, [`PublicBannerSlot`](src/components/public-banner-slot.tsx) renders above the header inside that stack.

### Banners

Admin-configurable surface banners backed by the settings registry — no separate persistence layer. Two keys in [`app-settings-registry.ts`](src/config/app-settings-registry.ts): `banner_public` (marketing) and `banner_authenticated` (signed-in app shell). Values are structured [`BannerSettingValue`](src/types/banner.ts) objects (mode `off` | `on` | `scheduled`, optional `starts_at` / required `expires_at` in scheduled mode, `headline` / optional `detail` with inline `**bold**` and `[text](url)` syntax, `variant`, `show_icon`). [`computeBannerStatus`](src/utils/banner-status.ts) / [`isBannerLive`](src/utils/banner-status.ts) derive `off` | `scheduled` | `live` from mode and timestamps — only `live` renders. [`parseBannerMessage`](src/utils/parse-banner-message.ts) splits headline/detail into text, bold, and link segments; relative paths and `http`/`https` links render clickable, other schemes render as plain text; [`BannerMessage`](src/components/banner-message.tsx) renders segments safely (no raw HTML). Shared [`AppBanner`](src/components/app-banner.tsx) applies variant-tinted bar styling and an optional knockout icon badge (Megaphone / Check / AlertTriangle / AlertCircle / Info per variant). Layouts read via [`getResolvedAppSettings`](src/utils/app-settings.ts) (registry-key fingerprint busts cache when keys change). [`PublicBannerSlot`](src/components/public-banner-slot.tsx) sits above marketing chrome; [`AuthenticatedBannerSlotEntry`](src/components/authenticated-banner-slot-entry.tsx) renders [`AuthenticatedBannerSlot`](src/components/authenticated-banner-slot.tsx) in [`(app)/layout.tsx`](src/app/(app)/layout.tsx) above [`AppShell`](src/app/(app)/_components/app-shell.tsx) (layout sibling, not inside the shell). Public dismissal writes a cookie ([`banner-cookies.ts`](src/constants/banner-cookies.ts)) keyed by [`buildBannerDismissKey`](src/utils/banner-dismiss-hash.ts) (headline + detail hash); SSR reads the cookie via [`resolveLiveBannerSlot`](src/utils/banner-dismiss-cookie.ts) so dismissal survives reload until content changes. Authenticated dismissal is in-memory only — hides instantly on close and reappears on the next page load.
...
tweakcn **Clean Slate** default theme in `src/app/globals.css`; semantic tokens via `@theme inline` + `next-themes` class-based light/dark — status colors include `destructive`, `success`, `warning`, and `info` (each with a foreground pair where applicable) across light and dark. Motion duration tokens (`duration-swept`, `duration-dwell`) apply a two-tier traversal-rate system to state transitions outside vendored `ui/` primitives — focus indicators are carved out and must appear instantly (never on an animated property list; tabs use `focus-visible:transition-none`) — see [DESIGN.md](DESIGN.md) § Motion; enforced by `local/motion-tier` ESLint ([`.cursor/rules/ui-styling.mdc`](.cursor/rules/ui-styling.mdc)). **Inter**, **Merriweather**, and **JetBrains Mono** via `next/font` (Merriweather on terms and privacy body copy). UI primitives in `src/components/ui/` (accordion, alert, alert-dialog, avatar, badge, breadcrumb, button, card, checkbox, dialog, dropdown-menu, form, input, label, separator, select, sheet, sidebar, skeleton, sonner, table, textarea, toggle, toggle-group, tooltip) — alert adds status-token variants (`info`, `success`, `warning`, `destructive`); sidebar implementation split under [`src/components/ui/sidebar/`](src/components/ui/sidebar/) and re-exported from [`sidebar.tsx`](src/components/ui/sidebar.tsx). See [DESIGN.md](DESIGN.md) for architecture and re-skin workflow.

### Error handling & feedback

Errors are classified by `kind: 'operational' | 'fault'` on [`AppError`](src/types/app-error.ts). [`AppErrorSurface`](src/components/app-error-surface.tsx) branches typed errors to [`InlineError`](src/components/inline-error.tsx) (operational) or [`ErrorPanel`](src/components/error-panel.tsx) (fault — destructive icon/message, neutral code chip when present, labeled Copy placing pretty-printed JSON with message and optional code/digest as separate keys via [`buildErrorCopyText`](src/components/error-panel.tsx) → [`buildStructuredCopyText`](src/utils/build-structured-copy-text.ts)) — auth forms, profile, password section, and admin user mutations use it rather than hand-written ternaries. Server actions return a typed response envelope (`{success, data}` / `{success: false, error}`). Route-level fault boundaries at [`src/app/admin/error.tsx`](src/app/admin/error.tsx) and [`src/app/auth/error.tsx`](src/app/auth/error.tsx) render `ErrorPanel` with retry and escape links. Toasts (sonner via shadcn [`Toaster`](src/components/ui/sonner.tsx) with icons from [`toast-icon-config`](src/utils/toast-icon-config.tsx), [`showSuccessToast`](src/utils/app-toast.ts)) are for success confirmations only — errors always render inline or in a panel, never as a toast (see [`.cursor/rules/error-handling.mdc`](.cursor/rules/error-handling.mdc)).
...
**Custom migrations:** 13 — [`20260622120000_create_profiles.sql`](supabase/migrations/20260622120000_create_profiles.sql), [`20260623120000_create_avatars_bucket.sql`](supabase/migrations/20260623120000_create_avatars_bucket.sql), [`20260715003530_admin_list_users.sql`](supabase/migrations/20260715003530_admin_list_users.sql), [`20260715004901_admin_list_users_banned_sort.sql`](supabase/migrations/20260715004901_admin_list_users_banned_sort.sql), [`20260715175218_admin_list_users_show_banned_filter.sql`](supabase/migrations/20260715175218_admin_list_users_show_banned_filter.sql), [`20260716041928_create_app_settings.sql`](supabase/migrations/20260716041928_create_app_settings.sql), [`20260717234520_create_app_logs.sql`](supabase/migrations/20260717234520_create_app_logs.sql), [`20260718135028_schedule_app_logs_retention_purge.sql`](supabase/migrations/20260718135028_schedule_app_logs_retention_purge.sql), [`20260718191656_app_logs_read_state_and_context_text.sql`](supabase/migrations/20260718191656_app_logs_read_state_and_context_text.sql), [`20260719042859_admin_list_users_tile_filters_and_stats.sql`](supabase/migrations/20260719042859_admin_list_users_tile_filters_and_stats.sql), [`20260720030842_app_logs_realtime_publication.sql`](supabase/migrations/20260720030842_app_logs_realtime_publication.sql), [`20260720142910_admin_list_users_new_30d_filter.sql`](supabase/migrations/20260720142910_admin_list_users_new_30d_filter.sql), [`20260720151459_revoke_client_execute_trigger_functions.sql`](supabase/migrations/20260720151459_revoke_client_execute_trigger_functions.sql)

| Entity | Table / bucket | Notes |
| ------ | -------------- | ----- |
| User | `auth.users` | Supabase built-in; available via auth |
| App settings | `public.app_settings` | Admin-editable runtime configuration keyed by the code registry ([`app-settings-registry.ts`](src/config/app-settings-registry.ts)). Columns: `key` (PK), `value` (jsonb), `updated_at`. Absent keys resolve to registry defaults at read time. Registry spans logging settings (level, retention) and banner settings (`banner_public`, `banner_authenticated` — structured banner objects). Admin-only RLS (SELECT/INSERT/UPDATE). Reads: [`resolveAppSettings`](src/utils/app-settings.ts) (uncached, service client — CLI and non-Next callers) and [`getResolvedAppSettings`](src/utils/app-settings.ts) (tagged `unstable_cache` per [ADR-0006](docs/adr/ADR-0006-settings-reads-cached-under-one-coarse-tag.md); cache key includes a sorted registry-key fingerprint). Saves: [`saveAppSettingAction`](src/app/admin/settings/_lib/actions.ts). Types: `AppSettingKey` and `AppSettingValueMap` derived from the registry constant (re-exported via [`app-settings.ts`](src/types/app-settings.ts)); value-shape primitives in [`app-settings.ts`](src/types/app-settings.ts) and [`banner.ts`](src/types/banner.ts) — add a registry entry only, no hand-synced key union. |
| App logs | `public.app_logs` | Durable application log rows written by [`persistAppLogRow`](src/utils/persist-app-log.ts) (service client INSERT; failures silent-drop). Columns: `id` (identity PK), `level` (`debug`/`info`/`warn`/`error`), `tag`, `message`, `context` (jsonb, nullable), `read_at` (timestamptz, null = unread), `context_text` (generated stored text from `context` for search only — not returned in admin row shape), `created_at`. Admin-only SELECT and UPDATE RLS — no authenticated INSERT/DELETE policies. Indexes: `(created_at desc, id desc)` for cursor paging; partial `(created_at desc, id desc) where read_at is null` for unread triage. Realtime: member of `supabase_realtime` publication (INSERT broadcasts only — admin logs live feed). Rows older than `log_retention_days` are purged daily at 03:00 UTC by `pg_cron` via `purge_expired_app_logs()` (window read from `app_settings` at job time; registry default when unset). Admin filtered list, mark-all-read, and unread-count queries share [`applyLogListFilters`](src/app/admin/logs/_lib/log-list-filters.ts) via [`list-app-logs.ts`](src/app/admin/logs/_lib/list-app-logs.ts) and [`mark-app-logs-read.ts`](src/app/admin/logs/_lib/mark-app-logs-read.ts). Wrappers: [`appLog`](src/utils/app-logger.ts), [`cliLog`](src/utils/app-logger-cli.ts). Types: [`app-logs.ts`](src/types/app-logs.ts). |
| Admin user list | `public.admin_list_users` | `SECURITY DEFINER` RPC — paginated, sortable admin listing over `auth.users` (email, verification, created, last sign-in, role, `banned_until`); optional `p_filter_unverified`, `p_filter_banned`, and `p_filter_new_30d` tile filters (all false = unfiltered; may be active together; unified `is_verified` / `is_currently_banned` / `is_new_30d` predicates shared by filters and sort; new-30d window is rolling 30 days from `now()` at query time); in-function admin gate via `auth.jwt()` `app_metadata.role`; static sort allowlist; `EXECUTE` granted to `authenticated` only. Called from [`list-admin-users.ts`](src/app/admin/users/_lib/list-admin-users.ts). |
| Admin user stats | `public.admin_user_stats` | `SECURITY DEFINER` RPC — global total, unverified, currently-banned, and new-30d counts over `auth.users` for users page stat tiles; same admin gate and verification/banned/new-30d predicates as `admin_list_users`; counts unaffected by list filters or search; `EXECUTE` granted to `authenticated` only. Called from [`list-admin-user-stats.ts`](src/app/admin/users/_lib/list-admin-user-stats.ts). |
...
| Path | Purpose |
| ---- | ------- |
| `src/app/(marketing)/` | Public marketing routes (`/`, `/reference`, `/features`, `/workflow`, `/terms`, `/privacy`) |
| `src/app/(app)/` | Authenticated user surfaces (`/home`; profile settings modal) with shared marketing chrome |
| `src/app/admin/` | Admin console (`/admin`, `/admin/users`, `/admin/logs`, `/admin/settings`) |
...
| `src/config/` | Product identity, landing content, and features inventory — re-skin surface |
...
| `src/proxy.ts` | Next.js auth proxy convention entry (delegates to `src/supabase/proxy.ts`) |
