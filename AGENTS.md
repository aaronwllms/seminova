...
Security headers via [`src/utils/security-headers.ts`](src/utils/security-headers.ts), wired through [`next.config.ts`](next.config.ts); CSP is report-only by default — flipping to enforcing (`CSP_ENFORCE=true`) requires nonce-based script handling, not just directive tightening (see the `// debt:` marker in that file). Avatar and redirect handling are covered under [Profile / account settings](#profile--account-settings) and [Auth & session](#auth--session) above (`isOwnedAvatarStorageUrl`, `isSafeRedirect`).
