...
| ------------------ | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Routes & layouts   | 10 pages, 5 layouts, 1 route handler | `src/app/(marketing)/`, `src/app/(app)/profile/`, `src/app/auth/**`, `src/app/admin/**`; `src/proxy.ts` → `src/supabase/proxy.ts`                             |
...
- **W1 — Auth & routing:** Public routes are `/` and `/auth/**` only per `src/supabase/proxy.ts`. Unauthenticated users redirect to `/auth/login`. Admin paths redirect non-admins to `/profile` at proxy and again in `AdminAuthGate`. `/auth/confirm` uses `isSafeRedirect` for optional `next` param. Production proxy fails closed when Supabase env is missing (`src/proxy.ts` + `scripts/checks/supabase-env.mjs` on build).
