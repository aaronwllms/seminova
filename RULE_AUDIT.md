...
**Remediation:** 2026-07-03 (pass 4 — `pm-collaboration.mdc` citation exception, RA-021 / C-004 resolved; pass 3 — project-standards, git-workflow, README, RLS links, RA-020)  
**Standard:** [`.cursor/skills/rule-authoring/SKILL.md`](.cursor/skills/rule-authoring/SKILL.md)  
**Scope:** All 27 files in `.cursor/rules/*.mdc`, cross-checked against `AGENTS.md` § Hard constraints  
...
1. **UI rules (`ui-styling`, `ui-accessibility`, `ui-shadcn`) carry redundant code examples** — several multi-block tutorials where file references and one principle would suffice (RA-011 open; RA-009–RA-010 resolved — demoted guidance now canonical in `.mdc` after the hard-constraints restructure).
2. **`rule-authoring` ownership table incomplete** — omits `logging.mdc`, `notifications.mdc`, `forms.mdc`, `data-tables.mdc` (RA-019; meta, next maintenance pass).
3. **Pass-3/4 remediations stable** — `project-standards.mdc`, `git-workflow.mdc`, README, RLS links, `security.mdc` Input Validation merge, `pm-collaboration.mdc` citation exception (RA-021 / C-004).
4. **Accepted dispositions unchanged** — `testing.mdc` length (RA-006), SQL verbatim templates (RA-008), `code-minimalism.mdc` word count (RA-016), broad `forms.mdc` / `notifications.mdc` globs deliberate.
...
| RA-009 | RESOLVED | `ui-styling.mdc`                                          | Signal-to-noise                        | § Responsive Design, § Dark Mode, § Best Practices — redundant responsive/dark-mode examples and checklist (lines 12–16, 78–122, 159–167).                                          | **Resolved:** hard-constraints restructure; structure/theme and mobile-first guidance merged into `ui-styling.mdc` as canonical home. Optional incremental trim deferred. |
| RA-010 | RESOLVED | `ui-accessibility.mdc`                                    | Signal-to-noise                        | § Custom Component Accessibility, § WCAG 2.1 AA Compliance — generic WCAG tutorial blocks (lines 32–151, 173–179).                                                                  | **Resolved:** hard-constraints restructure; WCAG 2.1 AA principle merged into `ui-accessibility.mdc` as canonical home. Optional incremental trim deferred.               |
...
| RA-021 | RESOLVED | `pm-collaboration.mdc`                                    | Rule hierarchy / contradiction         | § Communication previously banned all code blocks in chat, colliding with user-rule filepath citation syntax.                                                                       | **Pass 4:** navigation citations exempted; ban scope is code content, not citation syntax.                                                                                |
...
### C-004 — Chat code blocks vs code citations — RESOLVED (pass 4)

- **Rules:** `pm-collaboration.mdc` § Communication vs user rules ("Use code citation blocks… `startLine:endLine:filepath`")
- **Resolution:** `pm-collaboration.mdc` § Communication now bans code content in chat but explicitly requires filepath citation blocks as navigation references. Ban scope is code content, not citation syntax.
...
| `pm-collaboration.mdc`         | Correct always-on placement; RA-021 / C-004 resolved — citation navigation exempted (pass 4).        |
...
1. **UI rule trim pass (RA-009–RA-011)** — When to schedule incremental signal-to-noise cuts on `ui-styling`, `ui-accessibility`, `ui-shadcn`?
2. **`rule-authoring` ownership table (RA-019)** — Extend on next quarterly maintenance pass?
3. **`typescript.mdc` broad globs (`**/_.ts`, `\*\*/_.tsx`)** — Attaches alongside `security.mdc`, `supabase.mdc`, `forms.mdc`, and `notifications.mdc` on most edits. Likely intentional; watch for context-budget pressure if more broad globs are added.

**Closed in pass 4:**

- `pm-collaboration.mdc` chat code ban → navigation citations exempted; ban scope is code content, not citation syntax (RA-021 / C-004).
...
- `pm-collaboration.mdc` — § Communication exempts filepath citation blocks as navigation; code content ban unchanged
...
1. Incremental signal-to-noise passes on UI rules (RA-009–RA-011) as bandwidth allows
2. Extend `rule-authoring` ownership table (RA-019) on next maintenance pass
