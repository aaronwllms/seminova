...
**Prior remediation:** 2026-07-02 (priority + contradictions pass); 2026-07-02 (pass 2 — NEW findings)  
...
2. **`code-minimalism.mdc` and `pm-collaboration.mdc` exceed Always Apply word budget** — ~435 and ~309 words vs ~150-word target; load on every request (RA-005, RA-006) **DEFERRED**
...
**Remediation holdover:** Priority pass (RA-001–015, C-001–C-003) and pass 2 (C-004, RA-026–032, rules README refresh) remain **RESOLVED**. No regressions found on activation modes, brace-expansion globs, or stale schema claims.
...
| RA-006 | `pm-collaboration.mdc`                          | **DEFERRED** | Size budgets (Always Apply)      | ~51 lines / ~309 words vs budget                                                                                                      | Compress partnership contract; keep intent                                                    |
...
| `pm-collaboration.mdc`         | Intentional Always Apply partnership contract; size trim deferred (RA-006).                                        |
...
2. **`pm-collaboration.mdc` Always Apply** — **Open** for template forks; kept as-is; size trim deferred (RA-006).
...
**Next priority (deferred pass):** RA-005/006 Always Apply size trims; RA-016–022/025 signal-to-noise and dedup trims.
