...
**Purpose:** Invariant doc-maintenance procedure governing the planning docs ([ROADMAP.md](../ROADMAP.md), the PRDs in [prds/](prds/), and the frozen [archive/](archive/)). This is not project state — it applies to every product built from this template. Governs the planning skills (`project-kickoff`, `write-product-brief`, `promote-backlog-item`, `phase-planning`, `kickoff-phase`, `plan-next-epic`, `mark-epic-complete`, `ship-phase`), the research skills (`research`, `archive-research`), and the repo-sync skill (`sync-repo-docs`).
...
| **[prds/](prds/)** | PM / agents | Per-phase forward intent (problem, goal, scope); epics + stories, accumulating from `Planning` and complete at `Ready`. Lifecycle in [prds/README.md](prds/README.md) |
...
16. **The `nextjs-agent-rules` region in AGENTS.md is framework-owned.** The block delimited by `<!-- BEGIN:nextjs-agent-rules -->` / `<!-- END:nextjs-agent-rules -->` in [AGENTS.md](../AGENTS.md) is written and updated automatically by `next dev`. Do not hand-edit it. Changes to this region on a Next.js upgrade are expected, not documentation drift.
