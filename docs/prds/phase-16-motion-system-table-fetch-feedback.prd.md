...
- **Token block placement.** The existing color tokens indirect through `@theme inline` because they vary by light/dark. Duration tokens have no such variance, so the simpler plain form is likely correct — confirm against the current `globals.css` structure at build time.
