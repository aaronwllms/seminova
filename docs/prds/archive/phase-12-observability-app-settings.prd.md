...
- **10.1 The session seam.** Session and proxy decisions emit debug logs — token refreshed versus reused, claims read while expired — so the boundary [ADR-0005](../adr/ADR-0005-proxy-session-gate-two-authority-refresh.md) settled can be watched live rather than inferred from a stack trace.
