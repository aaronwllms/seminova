...
export function compareWiredChecks(scripts, workflowText) {
  ...
  const missingFromCi = checkNames.filter(
    (name) => !isInvoked(workflowText, name),
  )

  return {
    ok: missingFromPrePush.length === 0 && missingFromCi.length === 0,
    checkNames,
    missingFromPrePush,
    missingFromCi,
  }
...
if (isMain) {
  ...
  const result = compareWiredChecks(scripts, workflow)

  if (result.checkNames.length === 0) {
    fail('No "check:*" scripts found in package.json.')
  }

  if (!result.ok) {
    if (result.missingFromPrePush.length > 0) {
      console.error(
        `[check:checks-wired] Defined but not run by "pre-push": ${result.missingFromPrePush.join(', ')}`,
      )
    }

    if (result.missingFromCi.length > 0) {
      console.error(
        `[check:checks-wired] Defined but not run by CI (.github/workflows/pull-request.yaml): ${result.missingFromCi.join(', ')}`,
      )
    }
  ...
  console.log(
    `[check:checks-wired] OK — all ${result.checkNames.length} check scripts run in pre-push and CI.`,
  )
}
